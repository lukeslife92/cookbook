...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Recipe.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Recipe.updateOne(params, data, { multi: true });
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Recipe.attributes).reduce((acc, curr) => {
      switch (Recipe.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
